...
helpers turn it into structures without guessing at anything: a cell that the
source left blank, or marked ``Not listed`` / ``N/A`` / ``--``, comes back as
``None`` so downstream code can preserve the gap instead of inventing a value.
"""
...
def is_missing(value: str | None) -> bool:
    return clean(value or "") is None


def _split_row(line: str) -> list[str]:
    """Split one ``|``-delimited row, honouring ``\\|`` escapes."""
    stripped = line.strip()
...
class Table:
    ...
    #: Line number (0-based, into the text the table was parsed from) of the header.
    line: int = 0
...
@dataclass
class Section:
    """A heading and the body text that runs until the next heading of any level."""
...
def iter_sections(text: str, level: int | None = None) -> Iterator[Section]:
    ...
    ``level`` restricts the yielded headings to that depth; the body of each
    still runs to the next heading at the same *or shallower* depth, so a
    ``##`` section keeps its nested ``###`` subsections.
    """
    lines = text.splitlines()
...
def find_section(text: str, *, title_matches, level: int | None = None) -> Section | None:
    """First section whose title satisfies ``title_matches`` (a predicate)."""
