...
export function PortalLane({
  ...
  return (
    <section className="apex-lane" aria-labelledby="portal-title">
      <LaneHeading
        id="portal-title"
        eyebrow="NEW THREADS. NEW POSSIBILITIES."
        title="Portal movers"
        href="/transfer-portal"
        linkLabel="Track the portal"
      />
      <div className="apex-portal-grid">
        {rows.length === 0 && (
          <p className="apex-empty">Transfer counts not published.</p>
        )}
        {rows.map((row) => {
              ...
              <Image
                ...
                alt={
                  stadium.imageIsIllustration
                    ? "Illustrative college football stadium atmosphere, not a photograph of this venue"
                    : stadium.name
                }
                width={1024}
                height={1024}
