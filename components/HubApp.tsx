...
function PolicyPage({ kind }: { kind: string }) {
  const content: Record<string, { eyebrow: string; title: string; intro: string; sections: [string, React.ReactNode][] }> = {
    ...
    "affiliate-disclosure": {
      ...
      intro: "Ticket partner links (TicketNetwork, TicketSmarter) are live across the watch guide, team pages, and stadium guides. Affiliate approval is pending: until it clears, links go directly to the partners' public ticket searches and we earn no commission. Once approved, the same links convert to tracked referrals automatically.",
      ...
      <div className="apex-container">
        <div className="apex-home-intro">
          <div>
            ...
            <h1>{hub.name}</h1>
            <p>{hub.teams.length} programs · {hub.scheduledGames.filter((g) => g.status === "final").length} played · {hub.scheduledGames.filter((g) => g.status === "scheduled").length} scheduled</p>
          </div>
