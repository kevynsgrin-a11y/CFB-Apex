...
/** Unlike the score feed, a team schedule retains byes, FCS games and unreported results. */
export function getTeamSchedule(slug: string): TeamScheduleGame[] | null {
  ...
  return rows.map((row, index) => {
    const played = playedGames.find((game) =>
      game.date === row.date &&
    ...
    const displayGame = games.find((game) =>
    ...
    return {
      ...
      isBye: row.type === "bye",
      result: ownScore != null && otherScore != null
